...
//N
const totRighePadre = 4;    //Totale righe padre
...
function servizio(url){
    
    var xhttp = new XMLHttpRequest();   //Oggetto http request
    // var url = "http://localhost:8080/places";   //url server a cui si fa richiesta
...
function creaPadri(JSON){
    ...
    */
    ...
    //viewMore.onclick= caricaDetails(JSON);
...
function caricaDetails(JSON){
    //Switch
    var contenitoreHotel = document.getElementById("hotel");
    contenitoreHotel.style="display:none";

    var contenitoreDetails = document.getElementById("details");
    contenitoreDetails.style="display:block";

    var nome = document.createElement('h1');
    var contenuto = document.createTextNode(JSON.name);

    nome.appendChild(contenuto);
    contenitoreDetails.appendChild(nome);
}


function inserisciHotel(arrayDiJSON){
    // Svuota le tabelle
    document.getElementById("servizio").innerHTML= "";
    //Griglia
